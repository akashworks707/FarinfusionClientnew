...
// Must match backend dateFieldMap keys exactly
...
// Matches backend dateFieldMap keys → human labels
const DATE_TYPES: { value: DateType; label: string }[] = [
  { value: "created", label: "Order Created" },
  { value: "confirmed", label: "Confirmed At" },
  { value: "courierAssigned", label: "Courier Assigned" },
  { value: "pickedUp", label: "Picked Up" },
  { value: "delivered", label: "Delivered At" },
  { value: "partial", label: "Partial Delivered" },
  { value: "cancelled", label: "Cancelled At" },
  { value: "hold", label: "On Hold At" },
];
...
export function OrderFilters({
  ...
  const handleCalendarSelect = (range: DateRange | undefined) => {
    ...
    if (range?.from && range?.to) {
      onDateChange({ from: startOfDay(range.from), to: endOfDay(range.to) });
  ...
  return (
    <div className="rounded-xl border border-gray-200/80 bg-white p-4 dark:border-gray-700/60 dark:bg-gray-900 space-y-3">
      {/* Row 1: Search + Status + Delivery Status */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
        ...
        {/* Date type selector + date picker — grouped together */}
        <div className="flex shrink-0 items-center gap-0 rounded-lg border border-gray-200 bg-gray-50/60 dark:border-gray-700 dark:bg-gray-800/60 overflow-hidden">
          ...
          <Select
            value={dateType}
            onValueChange={(val) => onDateTypeChange(val as DateType)}
          >
            <SelectTrigger className="h-10 w-40 rounded-none border-0 border-r border-gray-200 dark:border-gray-700 bg-transparent text-xs font-medium text-gray-600 dark:text-gray-400 focus:ring-0 focus:border-r-gray-200 px-3">
              ...
              <div className="flex flex-col sm:flex-row">
                ...
                <div className="p-3">
                  ...
                  <Calendar
                    ...
                    classNames={{
                      day_selected:
                        "bg-amber-500 text-white hover:bg-amber-500 focus:bg-amber-500",
                      day_range_middle:
                        "bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300",
                      day_range_start:
                        "bg-amber-500 text-white rounded-l-full",
                      day_range_end: "bg-amber-500 text-white rounded-r-full",
                      day_today:
